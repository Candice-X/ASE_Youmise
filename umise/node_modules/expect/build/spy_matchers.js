...
const spyMatchers = {
  ...
  nthCalledWith(received, nth) {
    ...
    if (typeof nth !== 'number' || parseInt(nth, 10) !== nth || nth < 1) {
      const message = () => `nth value ${(0, _jestMatcherUtils.printReceived)(nth)} must be a positive integer greater than ${(0, _jestMatcherUtils.printExpected)(0)}`;
      const pass = false;
      return { message, pass };
    }
    ...
    const receivedName = receivedIsSpy ? 'spy' : received.getMockName();
    const calls = receivedIsSpy ? received.calls.all().map(x => x.args) : received.mock.calls;
    const pass = (0, _jasmine_utils.equals)(calls[nth - 1], expected, [_utils.iterableEquality]);
